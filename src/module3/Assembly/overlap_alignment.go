...
func ScoreOverlapAlignment(str0, str1 string, match, mismatch, gap float64) float64 {
    scoringMatrix := OverlapScoreTable(str0, str1, match, mismatch, gap)
...
func OverlapScoreTable(str0, str1 string, match, mismatch, gap float64) [][]float64 {
    numRows := len(str0) + 1
    numCols := len(str1) + 1
    scoringMatrix := InitializeScoreTable(numRows,numCols)
    // init 0th row
    for i := range(scoringMatrix[0]) {
      scoringMatrix[0][i] = float64(i) *(-gap)
    }

    for i := 1; i < numRows; i++ {
      for j := 1; j < numCols; j++ {
        up := scoringMatrix[i-1][j] - gap
    ...
    scoringMatrix[numRows - 1][numCols - 1] = MaxArrayFloat(scoringMatrix[numRows-1])
